// Package store provides bwx's workspace-aware storage interface plus its
// local file-backed and cloud (gl-svc HTTP) implementations.
//
// bwx adds a per-call journal id everywhere a write happens, so a workspace
...
// Store is the bwx-flavored ledger store. The journal id is passed through
