...
import (
	...
	"github.com/bitwave-io/bitwave-cli/internal/bwx/shares"
...
Auth:
  Local mode share is ANONYMOUS — no ` + "`bwx auth login`" + ` required. gl-svc
  ...
  on behalf of your org, so ` + "`bwx auth login`" + ` + ` + "`bwx org use`" + ` must already
  be set up.
...
Examples:
  bwx share --to teammate@example.com
  bwx share --to teammate@example.com --message "May expenses for review"
  bwx share --to teammate@example.com --ttl 24       # link expires in 24h
  bwx share --to teammate@example.com --dry-run      # don't actually send`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if cfg.OrgId == "" {
				return fmt.Errorf(".bwx.toml is missing org_id for cloud share")
			}
...
func newSharesListCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if cfg.OrgId == "" {
				return fmt.Errorf(".bwx.toml is missing org_id")
			}
			s, _, _, err := resolveStore(cmd.Context())
			if err != nil {
				return err
