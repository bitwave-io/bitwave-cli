...
// resolveStore builds a bwx store rooted at the cwd's .bwx.toml. Most ledger
...
func resolveStore(ctx context.Context) (store.Store, *config.Config, string, error) {
	...
	if err != nil {
		if errors.Is(err, config.ErrNotAWorkspace) {
			return nil, nil, "", fmt.Errorf("%w — run `bwx init` here (or `cd` to an existing workspace) before this command", err)
		}
		return nil, nil, "", err
	}
