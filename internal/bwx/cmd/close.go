...
// newCloseRunStubCmd is a placeholder for bwx close run. The orchestrator
...
func newCloseRunStubCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return fmt.Errorf("bwx close run is not yet wired up — use `bw close run` (same flags). The orchestrator will move into a shared package in a follow-up")
		},
	}
}
