...
func newOrgCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Cloud bwx commands inherit an active org from ~/.bw/config.json. Use
` + "`bwx org use`" + ` / ` + "`bwx org create`" + ` to manage it instead of passing
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if err != nil {
				if errors.Is(err, orgctx.ErrNoActiveOrg) {
					fmt.Fprintln(os.Stderr, "No active org. Run `bwx org use` to pick one.")
					os.Exit(1)
				}
				return err
			}
...
func newOrgUseCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(list) == 0 {
				fmt.Println("You have no orgs. Run: bwx org create --name <name>")
				return nil
			}
