...
import (
	...
	"github.com/bitwave-io/bitwave-cli/internal/bwx/workspaceshare"
)
...
func newWorkspaceCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `bwx workspace lists, creates, and switches between cloud workspaces in
the active org. The selection is recorded in the cwd's .bwx.toml so
later commands (bwx je, bwx bal, ...) target the right workspace.`,
	}
...
// flips back to active when it completes; `bwx workspace list` will then
...
func newWorkspaceAdoptCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Printf("Hydration runs asynchronously; use `bwx workspace list` to see when it appears.\n")
			return nil
		},
	}
...
func newWorkspaceListCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if len(ws) == 0 {
				fmt.Println("(no workspaces) — run `bwx init --cloud --name <n>` to create one")
				return nil
			}
...
func newWorkspaceCurrentCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "current",
		Short: "Print the workspace recorded in the cwd's .bwx.toml",
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if cfg.WorkspaceId == "" {
				return fmt.Errorf("no workspace_id in .bwx.toml — run `bwx workspace use`")
			}
...
func newWorkspaceUseCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "use [workspaceId]",
		Short: "Point the cwd .bwx.toml at a cloud workspace. Bare invocation shows a picker.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 1 {
				...
				if len(ws) == 0 {
					fmt.Println("No workspaces. Run: bwx init --cloud --name <name>")
					return nil
				}
...
func newWorkspaceCreateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `bwx workspace create makes an empty cloud workspace under the active
org. It does NOT touch cwd's .bwx.toml — for that use ` + "`bwx init --cloud`" + `
...
// loadCwdConfig finds the nearest .bwx.toml from cwd and returns it.
func loadCwdConfig() (*config.Config, string, error) {
	...
	if err != nil {
		if errors.Is(err, config.ErrNotAWorkspace) {
			return nil, "", fmt.Errorf("%w — run `bwx init` here (or `cd` to an existing workspace) before this command", err)
		}
		return nil, "", err
	}
