...
func newMigrateCmd() *cobra.Command {
  ...
  3. Rewrite .bwx.toml to mode=cloud + the new ids
...
via the bwx delegation flow. For now this errors out — pass --invite once
the server-side delegation endpoint lands.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if invite != "" {
				return fmt.Errorf("--invite requires the bwx delegation flow; not yet implemented")
			}
			cwd, err := os.Getwd()
			if err != nil {
				return err
