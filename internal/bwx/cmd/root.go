// Package cmd defines the bwx CLI command tree.
//
// bwx is a reshape of bw focused on plain-text accounting workflows that an
...
// NewRootCmd builds the bwx root command tree.
func NewRootCmd() *cobra.Command {
	root := &cobra.Command{
		Use:   "bwx",
		Short: "Bitwave plain-text accounting CLI",
		Long: `bwx is the agent-friendly Bitwave CLI for plain-text accounting (PTA).

Every bwx command operates on a workspace — a directory containing a
.bwx.toml marker plus one or more .journal files. Run ` + "`bwx init`" + ` in the
directory you want the workspace to live in BEFORE any other command. bwx
walks up from the cwd to find the workspace marker, so you can run
commands from any subdirectory once it exists.

Modes:
  - Local (default): files live on disk. No auth needed. ` + "`bwx share`" + ` also
    works anonymously in local mode (the recipient is the one who signs in
    to adopt the workspace).
  - Cloud (` + "`bwx init --cloud`" + `): backed by gl-svc under your org. Requires
    ` + "`bwx auth login`" + ` and ` + "`bwx org use`" + `.

Auth (used by cloud-mode commands; priority order):
  - BITWAVE_AGENT_TOKEN env  Well-known agent identity
  - bwx auth login           Human PKCE browser flow
  - bwx auth delegate        Request delegated access from a user

Operating context is printed to stderr before every command as a one-line
banner: ` + "`bwx: workspace=... | org=... | identity=...`" + `. Suppress with
--quiet or BWX_QUIET=1. Run ` + "`bwx status`" + ` to print it on demand.

Quickstart — log expenses and share a report:
  cd ~/my-expenses
  bwx init                                                 # workspace in cwd
  ...
  bwx expense report 2026-05                               # render to stdout
  bwx share --to me@example.com                            # email a link

Quickstart — raw double-entry journal:
  bwx init
  bwx acct add Assets:Cash
  bwx acct add Income:Salary
  bwx je new --date 2026-01-01 --payee "Initial" \
      --posting "Assets:Cash 1000 USD" \
      --posting "Income:Salary -1000 USD"
  bwx bal

Tip: run ` + "`bwx <command> --help`" + ` on any subcommand to see flags + examples.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	root.PersistentFlags().BoolVar(&quietFlag, "quiet", false, "Suppress the bwx status banner (also: BWX_QUIET=1)")
...
func newVersionCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "version",
		Short: "Print the bwx CLI version",
