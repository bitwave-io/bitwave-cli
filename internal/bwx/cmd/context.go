...
// makeTokenResolver returns a token resolver applying the bwx priority:
//
...
// (or just-completed `bwx auth login`) is picked up.
...
// if none is set. The hint mentions bwx, not bw.
func requireActiveOrg() (*orgctx.Active, error) {
	a, err := orgctx.Load()
	if err != nil {
		if errors.Is(err, orgctx.ErrNoActiveOrg) {
			fmt.Fprintln(os.Stderr, "No active org. Run `bwx org use` to pick one, or `bwx org create` to make a new one.")
		}
		return nil, err
	}
	return a, nil
