// Package workspaceshare is the bwx HTTP client for the gl-svc workspace
...
//
// The share path is intentionally unauthenticated — bwx in local mode has no
...
//
// Only `.bwx.toml`, `accounts.ledger`, `prices.ledger`, `*.journal`, and
...
func buildZip(workspaceDir string) (*bytes.Buffer, error) {
	...
	if included == 0 {
		return nil, fmt.Errorf("no workspace files found in %s (need .bwx.toml + at least one .journal/.ledger file)", workspaceDir)
	}
	return &buf, nil
}
...
func isWorkspaceFile(name string) bool {
	...
	if name == ".bwx.toml" || name == "accounts.ledger" || name == "prices.ledger" {
		return true
	}
