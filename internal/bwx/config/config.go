// Package config handles the per-workspace .bwx.toml file that records
// whether a workspace is local or cloud-backed.
//
// A bwx workspace is a directory containing:
//   - .bwx.toml        — this config (mode, currency, optional cloud ids)
...
// directory still holds .bwx.toml as the workspace marker so cwd-aware
...
// ErrNotAWorkspace is returned by Find/Load when no .bwx.toml is present.
var ErrNotAWorkspace = errors.New("not a bwx workspace (no .bwx.toml)")

// Find walks up from start looking for a .bwx.toml. Returns the workspace
...
// Save writes the config to <dir>/.bwx.toml with 0644 perms.
