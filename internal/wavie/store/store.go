// Package store provides wavie's workspace-aware storage interface plus its
// local file-backed and cloud (gl-svc HTTP) implementations.
//
// wavie adds a per-call journal id everywhere a write happens, so a workspace
...
// Store is the wavie-flavored ledger store. The journal id is passed through
