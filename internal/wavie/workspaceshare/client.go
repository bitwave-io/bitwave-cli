// Package workspaceshare is the wavie HTTP client for the gl-svc workspace
...
//
// The share path is intentionally unauthenticated — wavie in local mode has no
...
//
// Only `.wavie.toml`, `accounts.ledger`, `prices.ledger`, `*.journal`, and
...
func buildZip(workspaceDir string) (*bytes.Buffer, error) {
	...
	if included == 0 {
		return nil, fmt.Errorf("no workspace files found in %s (need .wavie.toml + at least one .journal/.ledger file)", workspaceDir)
	}
	return &buf, nil
}
...
func isWorkspaceFile(name string) bool {
	...
	if name == ".wavie.toml" || name == "accounts.ledger" || name == "prices.ledger" {
		return true
	}
