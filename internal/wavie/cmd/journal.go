...
` + "`wavie journal use`" + ` records a default journal id in .wavie.toml so
` + "`wavie je new`" + ` doesn't have to keep passing --journal.`,
	}
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case config.ModeCloud:
				if cfg.OrgId == "" || cfg.WorkspaceId == "" {
					return fmt.Errorf(".wavie.toml is missing org_id or workspace_id")
				}
...
func newJournalUseCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "use <id>",
		Short: "Set the default journal for `wavie je new` and other writes",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			id := strings.TrimSpace(args[0])
			cfg, dir, err := loadCwdConfig()
			...
			if !found {
				return fmt.Errorf("journal %s does not exist (run `wavie journal new %s`)", id, id)
			}
...
func listJournals(cfg *config.Config, dir string) ([]string, []string, error) {
	...
	case config.ModeCloud:
		if cfg.OrgId == "" || cfg.WorkspaceId == "" {
			return nil, nil, fmt.Errorf(".wavie.toml is missing org_id or workspace_id")
		}
		c := workspaces.New(resolveGLBaseURL(), cfg.OrgId, makeOrgTokenResolver(cfg.OrgId))
