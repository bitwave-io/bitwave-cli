...
Each ` + "`wavie expense new`" + ` writes one balanced double-entry transaction:
...
Examples:
  ...
  wavie expense new --report Q1-travel --date 2026-05-16 \
  ...
  wavie expense report 2026-05              # human-readable table on stdout
  wavie expense report 2026-05 --format csv # csv|qif|html|json for export

To email a recipient a tokenized link to the workspace, use the root share:
  wavie share --to me@example.com`,
	}
...
tagged with expense-report:<--report> so it shows up in ` + "`wavie expense report`" + `.
...
Examples:
  wavie expense new --report 2026-05 --amount 10 --account Expenses:Meals
  wavie expense new --report Q1 --amount 120 --account Expenses:Travel \
      --merchant Acme --reimbursable
  wavie expense new --report 2026-05 --amount "1 ETH" \
