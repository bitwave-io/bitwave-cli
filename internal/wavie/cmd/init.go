...
func newInitCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "init",
		Short: "Scaffold a wavie workspace in the current (or --dir) directory",
		Long: `wavie init creates a .wavie.toml marker in the current directory (or --dir),
plus empty accounts.ledger / prices.ledger files for local mode. This must
run BEFORE any other wavie command — most commands fail with "not a wavie
workspace" until a .wavie.toml exists at or above the cwd.

The workspace lives where you run init. Run it from inside the directory
you want to use (e.g. ` + "`cd ~/my-expenses && wavie init`" + `), or pass --dir to
scaffold somewhere else. Workspace name defaults to the directory basename.

Local mode (default):
  wavie init [--name N] [--base-currency USD]
    Writes .wavie.toml plus empty accounts.ledger / prices.ledger.

Cloud mode (--cloud):
  wavie init --cloud --name N [--base-currency USD]
    Creates a LedgerWorkspace under the active org and binds the cwd to it.
    Requires ` + "`wavie auth login`" + ` and an active org (` + "`wavie org use`" + `).

Examples:
  cd ~/my-expenses && wavie init
  wavie init --dir ./jan-2026 --name jan-expenses
  wavie init --base-currency EUR
  wavie init --cloud --name acme-fy26`,
...
func runInitLocal(dir, name, baseCurrency string) error {
	...
	fmt.Println("Next:")
	fmt.Println("  wavie expense new --report <id> --amount 10 --account Expenses:Meals")
	fmt.Println("  wavie je new   (raw double-entry)   |   wavie --help   (all commands)")
	return nil
}
...
func runInitCloud(dir, name, baseCurrency string) error {
	...
	if err := config.Save(dir, cfg); err != nil {
		return fmt.Errorf("save .wavie.toml: %w", err)
	}
