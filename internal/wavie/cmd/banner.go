...
// quietFlag is set by --quiet (and respects WAVIE_QUIET=1) to suppress the
...
// Honors --quiet and WAVIE_QUIET=1.
func printStatusBanner(cmd *cobra.Command) {
	if quietFlag || os.Getenv("WAVIE_QUIET") == "1" {
		return
	}
	path := cmd.CommandPath()
	// Skip the bare root (e.g. `wavie --help`) — it has no useful target.
	if path == "wavie" {
		return
	}
...
// `wavie status` command can reuse the same formatter.
func describeStatus() string {
	return fmt.Sprintf("wavie: %s | %s | %s",
		describeWorkspace(),
		describeOrg(),
		describeIdentity(),
	)
...
func describeWorkspace() string {
	...
	if err != nil {
		if errors.Is(err, config.ErrNotAWorkspace) {
			return "workspace=none (run `wavie init`)"
		}
		return "workspace=?"
	}
...
func describeIdentity() string {
	...
	if err != nil || creds == nil {
		return "identity=anonymous (no `wavie auth login` — fine for local mode)"
	}
