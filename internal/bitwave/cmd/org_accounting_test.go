...
func TestAccountingReadinessPromptsForSetup(t *testing.T) {
	readiness := buildAccountingReadiness(nil, nil)
...
func TestAccountingReadinessRequiresChartThenBecomesReady(t *testing.T) {
	connections := []orgreports.AccountingConnection{{ID: "ac-1", Type: "manual"}}
	readiness := buildAccountingReadiness(connections, nil)
	if readiness.Decision != "chart_of_accounts_required" || !readiness.InteractionRequired {
		t.Fatalf("readiness = %#v", readiness)
	}
