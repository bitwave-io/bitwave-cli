...
func newWorkspaceAdoptCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Printf("Hydration runs asynchronously; use `bitwave workspace list` to see when it appears.\n")
			return nil
		},
	}
...
func newWorkspaceListCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if len(ws) == 0 {
				fmt.Println("(no workspaces) — run `bitwave init --cloud --name <n>` to create one")
				return nil
			}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 1 {
				...
				if len(ws) == 0 {
					fmt.Println("No workspaces. Run: bitwave init --cloud --name <name>")
					return nil
				}
