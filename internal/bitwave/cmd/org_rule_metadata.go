...
func newRuleMetadataGuideCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "metadata-guide",
		Short: "Return Bitwave's documented Canton metadata mappings for an LLM",
		RunE: func(cmd *cobra.Command, _ []string) error {
			if limit < 1 || limit > 100 {
				return errors.New("--limit must be between 1 and 100")
			}
			chart = strings.ToLower(strings.TrimSpace(chart))
			if chart != "both" && chart != "standard" && chart != "specific" {
				return errors.New("--chart must be standard, specific, or both")
			}
			...
			return writeJSON(cmd.OutOrStdout(), map[string]any{
				...
				"source": guide.Source, "network": guide.Network, "operators": guide.Operators,
				"documentedKeys": guide.DocumentedKeys, "standardChart": standard,
				"standardSpecificChart": specific, "patterns": patterns,
				"vendorSpecificGuidance": guide.VendorSpecificGuidance,
				"internalTransferStatus": guide.InternalTransferStatus,
				"filters":                map[string]any{"key": key, "value": value, "chart": chart, "limit": limit},
			})
		},
	}
	...
	cmd.Flags().StringVar(&chart, "chart", "both", "Chart mapping: standard, specific, or both")
